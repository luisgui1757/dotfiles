#!/usr/bin/env bash
# Transactional v0.1.0 -> v0.4.4 migration for macOS, Linux, and WSL userland.
# Run this script only from a separate, exact v0.4.4 release checkout.
set -euo pipefail
...
annotated v0.4.4 release. The old v0.1.0 checkout remains untouched and is the
    ...
    case "$os" in
        Darwin)
            [[ "$arch" == "arm64" || "$arch" == "aarch64" ]] || {
                fail "the v0.4.4 macOS upgrade requires Apple Silicon (arm64); detected $arch."
                return 1
            }
...
run_preflight() {
    ...
    new_checkout="$(canonical_directory "$new_input")" || {
        fail "v0.4.4 checkout is not an existing real directory: $new_input"
        return 1
    }
    [[ "$old_checkout" != "$new_checkout" ]] || {
        fail "in-place migration is forbidden; retain v0.1.0 and use a separate v0.4.4 checkout."
        return 1
    }
...
prepare_recovery() {
    ...
    recovery="$(mktemp -d "$state_root/v0.1.0-to-v0.4.4.XXXXXX")"
    chmod 700 "$recovery"
    ...
    printf '%s\n' \
        "Private recovery material for v0.1.0 -> v0.4.4." \
...
load_recovery() {
    ...
    if [[ "$require_checkouts" -eq 1 ]]; then
        ...
        new_checkout="$(canonical_directory "$new_checkout")" || {
            fail "retained v0.4.4 checkout is missing or unsafe."
            return 1
        }
    ...
    cmp -s "$recovery/flake.lock" "$new_source/flake.lock" || {
        fail "captured flake.lock differs from the exact v0.4.4 release."
        return 1
    }
    cmp -s "$recovery/upgrade-v0.1.0.sh" "$new_source/scripts/upgrade-v0.1.0.sh" || {
        fail "recovery script differs from the exact v0.4.4 release."
        return 1
    }
...
perform_accept() {
    ...
    verify_new_state "$recovery" "$loaded_new_source" "$os" || {
        fail "v0.4.4 verification failed; keep both checkouts and recovery material."
        return 1
    }
    set_stage "$recovery" accepted
    echo "Migration core accepted. Keep v0.1.0 until full v0.4.4 setup succeeds."
...
perform_apply() {
    ...
    transaction_active=0
    trap - EXIT HUP INT TERM
    echo "v0.4.4 migration applied and verified."
...
case "$mode" in
    --preflight-only)
        ...
        echo "v0.1.0 -> v0.4.4 preflight passed; no live state changed."
        printf 'old=%s\nnew=%s\nplatform=%s\n' \
            "$preflight_old_checkout" \
            "$preflight_new_checkout" \
            "$preflight_platform"
