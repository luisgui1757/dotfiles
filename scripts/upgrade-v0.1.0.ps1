# Transactional v0.1.0 to v0.4.2 migration for native Windows.
# Run only from a separate, exact v0.4.2 release checkout.
...
function Invoke-UpgradePreflight {
    ...
    if ($old -ieq $new) {
        throw 'in-place migration is forbidden; retain v0.1.0 and use a separate v0.4.2 checkout'
    }
...
function Initialize-PrivateRecoveryDirectory {
    ...
    do {
        $recovery = Join-Path $Parent ("v0.1.0-to-v0.4.2." + [guid]::NewGuid().ToString('N'))
...
function Initialize-UpgradeRecovery {
    ...
    try {
        ...
        if ([IO.Path]::GetFullPath($script:WindowsIdentity.UserProfile) -ine
            ...
            throw 'frozen v0.4.2 release resolves a different Windows target identity'
        }
        ...
        @(
            'Private recovery material for v0.1.0 to v0.4.2.',
...
function Get-RecoveryState {
    ...
    if ($RequireCheckouts) {
        ...
        if ($newHeadResult.Stdout.Count -ne 1 -or $newHeadResult.Stdout[0] -ne $newCommit -or
            ...
            throw 'retained v0.4.2 checkout or tag moved; refusing acceptance'
        }
    ...
    if ((Get-FileSha256OrEmpty -Path (Join-Path $recovery 'upgrade-v0.1.0.ps1')) -ne
        ...
        throw 'recovery script differs from the exact v0.4.2 release'
    }
...
function Invoke-UpgradeAccept {
    ...
    if ($head -ne $state.NewCommit) { throw 'v0.4.2 checkout moved after migration' }
    if (-not (Test-NewConfig -NewCheckout $state.NewSource)) {
        throw 'v0.4.2 config verification failed; retain both checkouts and recovery material'
    }
    ...
    Write-Information 'Migration core accepted. Keep v0.1.0 until full v0.4.2 setup finalizes retained conventional targets.' -InformationAction Continue
    Write-Information 'Run full v0.4.2 setup now; archive v0.1.0 only after it succeeds.' -InformationAction Continue
}
...
function Invoke-UpgradeApply {
    ...
    try {
        ...
        if ($setupExitCode -ne 0) {
            throw "v0.4.2 setup exited $setupExitCode"
        }
        if (-not (Test-NewConfig -NewCheckout $frozen.NewSource)) {
            throw 'v0.4.2 config verification failed'
        }
    ...
    if ($applyFailure) {
        ...
        if ($setupExitCode -ne 0) {
            throw "v0.4.2 setup exited $setupExitCode; v0.1.0 was restored"
        }
        throw "$($applyFailure.Exception.Message); v0.1.0 was restored"
    }
    Write-Information 'v0.4.2 config migration applied and verified without package mutation.' -InformationAction Continue
...
switch ($PSCmdlet.ParameterSetName) {
    'Preflight' {
        ...
        Write-Information 'v0.1.0 to v0.4.2 preflight passed; no live state changed.' -InformationAction Continue
