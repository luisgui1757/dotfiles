...
#   git clone --branch v0.4.2 --single-branch https://github.com/luisgui1757/dotfiles.git "$env:USERPROFILE\dotfiles"
...
function Get-PendingV01Recovery {
    ...
    foreach ($directory in @(Get-ChildItem -LiteralPath $root -Force -Filter 'v0.1.0-to-v0.2.0.*' -ErrorAction SilentlyContinue)) {
        ...
        if ($stage -in @('prepared', 'applying', 'applied', 'rolling-back', 'recovery-required')) {
            throw "unfinished v0.2.0 migration must be resolved before v0.4.2 setup: recovery=$($directory.FullName); new-checkout=$newCheckout; old-checkout=$oldCheckout"
        }
    ...
    foreach ($directory in @(Get-ChildItem -LiteralPath $root -Force -Filter 'v0.1.0-to-v0.3.0.*' -ErrorAction SilentlyContinue)) {
        ...
        if ($stage -in @('prepared', 'applying', 'applied', 'rolling-back', 'recovery-required')) {
            throw "unfinished v0.3.0 migration must be resolved before v0.4.2 setup: recovery=$($directory.FullName); new-checkout=$newCheckout; old-checkout=$oldCheckout"
        }
        if ($stage -notin @('accepted', 'rolled-back')) {
            throw "legacy migration recovery stage is invalid: $($directory.FullName) ($stage)"
        }
    ...
    foreach ($directory in @(Get-ChildItem -LiteralPath $root -Force -Filter 'v0.1.0-to-v0.4.0.*' -ErrorAction SilentlyContinue)) {
        ...
        if ($stage -in @('prepared', 'applying', 'applied', 'rolling-back', 'recovery-required')) {
            throw "unfinished v0.4.0 migration must be resolved before v0.4.2 setup: recovery=$($directory.FullName); new-checkout=$newCheckout; old-checkout=$oldCheckout"
        }
        if ($stage -notin @('accepted', 'rolled-back')) {
            throw "legacy migration recovery stage is invalid: $($directory.FullName) ($stage)"
        }
    ...
    foreach ($directory in @(Get-ChildItem -LiteralPath $root -Force -Filter 'v0.1.0-to-v0.4.1.*' -ErrorAction SilentlyContinue)) {
        ...
        if ($stage -in @('prepared', 'applying', 'applied', 'rolling-back', 'recovery-required')) {
            throw "unfinished v0.4.1 migration must be resolved before v0.4.2 setup: recovery=$($directory.FullName); new-checkout=$newCheckout; old-checkout=$oldCheckout"
        }
        if ($stage -notin @('accepted', 'rolled-back')) {
            throw "legacy migration recovery stage is invalid: $($directory.FullName) ($stage)"
        }
