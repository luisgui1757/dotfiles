...
new_checkout="$WORK/dotfiles-v0.4.3"
home="$WORK/home"
bin="$WORK/bin"
mkdir -p "$home" "$bin"
...
if [[ "$*" == *"ls-remote --tags https://github.com/luisgui1757/dotfiles.git"* ]]; then
    case "$*" in
        ...
        *refs/tags/v0.4.3*)
            printf '%s\t%s\n' \
                "$TEST_NEW_TAG_OBJECT" refs/tags/v0.4.3 \
                "$TEST_NEW_COMMIT" 'refs/tags/v0.4.3^{}'
            ;;
        *) exit 91 ;;
    esac
    exit 0
...
[[ "$(python3 -c 'import os,sys; print(os.path.realpath(sys.argv[1]))' "$HOME/.zshrc")" == \
    ...
    fail "successful migration did not publish frozen v0.4.3 config"
[[ ! -e "$XDG_STATE_HOME/dotfiles/zsh-plugin-publisher.initialized" ]] ||
