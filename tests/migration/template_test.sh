...
# .config/{lazygit,ghostty} off-linux,
# .zshenv/.zshrc/.config/{nvim,herdr} on windows)
# is ignored whole so
...
expected_for() {
    case "$1" in
        ...
        windows)   printf '%s\n' ".config/aerospace" ".config/ghostty" ".config/herdr" ".config/lazygit" ".config/nvim" ".tmux.posix.conf" ".zshenv" ".zshrc" "AppData" "Documents" "Library" ;;
