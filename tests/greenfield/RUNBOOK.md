...
| `ls -la` a dir that has sub-folders | directory names are **gold**, long-list metadata follows Rose Pine, not blue-on-white | **`lsd` config + `$PSStyle`/LS_COLORS dir colour** |
