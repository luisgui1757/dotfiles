...
head_commit="$($REAL_GIT -C "$fixture" rev-parse HEAD)"
"$REAL_GIT" -C "$fixture" -c user.name=fixture -c user.email=fixture@example.invalid \
    tag -a v0.4.4 -m release
tag_object="$($REAL_GIT -C "$fixture" rev-parse refs/tags/v0.4.4)"
refs="$work/exact-release.refs"
{
    printf '%s\trefs/tags/v0.4.4\n' "$tag_object"
    printf '%s\trefs/tags/v0.4.4^{}\n' "$head_commit"
    printf '%s\trefs/heads/main\n' "$head_commit"
} > "$refs"
run_helper "$fixture" "$refs"
[[ "$rc" -eq 0 ]] || fail "exact annotated release was rejected:\n$output"
[[ "$output" == *"Verified immutable release checkout: v0.4.4 at $head_commit"* ]] ||
...
"$REAL_GIT" -C "$fixture" tag -d v0.4.4 >/dev/null
run_helper "$fixture" "$refs"
[[ "$rc" -ne 0 ]] || fail "official branch fallback remained open after release publication"
[[ "$output" == *"v0.4.4 is published; use a fresh exact-tag checkout"* ]] ||
...
[[ "$output" == *"local v0.4.4 does not match the official immutable annotated release"* ]] ||
...
[[ "$output" == *"official v0.4.4 must be one unique annotated tag"* ]] ||
