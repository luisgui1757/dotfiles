...
case "${1:-}" in
    pack)
        ...
        [[ -n "$dest" ]] || exit 52
        filename='earendil-works-pi-coding-agent-0.82.1.tgz'
...
#!/usr/bin/env bash
printf '%s\n' "${PI_TEST_INSTALLED_VERSION:-0.82.1}"
EOS
...
run_case() {
    local name="$1" mode="$2" reported="$3" installed_version="${4:-0.82.1}"
...
EOF
    ...
    [[ "$out" == *"already installed (0.82.1)"* ]] || fail "current Pi must be idempotent; got: $out"
)
...
EOF
    ...
    PI_TEST_INSTALLED_VERSION=0.82.1; export PI_TEST_INSTALLED_VERSION
...
grep -E '^install -g --prefix .*/dotfiles-pi\.[^/]*/earendil-works-pi-coding-agent-0\.82\.1\.tgz @earendil-works/pi-agent-core@0\.82\.1 @earendil-works/pi-ai@0\.82\.1 @earendil-works/pi-tui@0\.82\.1$' "$TMP_ROOT/success.log" >/dev/null \
...
# later on PATH, so setup installed 0.82.1 but verified Homebrew's stale 0.80.3.
(
...
EOF
    ...
    [[ "$(pi --version)" == "0.82.1" ]] || fail "wrong Pi wins after install"
