...
The published `v0.4.2` release path accepts only the exact clean official
annotated tag: object `807b2f8bb47ae9c7e132f1f8a218cf19c9ae2c61` peels to commit
`fdd628b34a58a3ecf3a1bef3de72f7cd4ac7dfc0`. Branch testing remains an explicit
...
Use this only for a greenfield or already-current-release test machine. Replace the
...
Retain the exact old checkout, clone the annotated v0.4.2 release beside it,
...
then completes additive provisioning and repoints config to v0.4.2. A pending
...
or v0.4.1 recovery from its retained exact release checkout before starting
v0.4.2; the new setup refuses to bypass any earlier transaction. macOS
...
**npm ownership on POSIX.** Nix owns the immutable Node/npm runtime; npm does
not write packages back into that runtime. During normal setup,
`install-deps.sh` uses npm's own user-config command to persist the global
prefix as `~/.local`, preserves existing registry/auth settings, and verifies
the effective prefix before continuing. Managed zsh already puts
`~/.local/bin` first on `PATH`. The canonical usage split is:

```bash
npm exec --package <package> -- <command>  # try/run a package without a global install
npm install                               # project dependency, local to the project
npm install --global <package>            # persistent personal CLI under ~/.local
```

Do not use `sudo npm install --global`, and do not overlay a second mutable Node
installation. Native Windows keeps npm's normal per-user Windows prefix.
...
equals one of the three canonical Rose Pine values or a retired repo-managed
`*-fable` trial alias, and never changes a later user choice.
...
- Pi CLI is a provisioned binary with two repo-owned preference surfaces
  ...
  uninstall command. Chezmoi deploys the canonical Fable-tuned
  `pi/rose-pine.json`, `pi/rose-pine-moon.json`, and
  `pi/rose-pine-dawn.json` themes and the exact `pi/keybindings.json` multiline pair
  (`Shift+Enter`, with `Ctrl+J` retained as Pi's transport fallback). The
  theme files preserve the variables and export roles from archived
  [`zenobi-us/pi-rose-pine`](https://github.com/zenobi-us/pi-rose-pine) commit
  `9b342f6e16d6b28c00c2f888ba2f050273981bdb`, with only Pi's current
  `earendil-works/pi` schema URL added. Its 15 base colors per variant are the
  official Rose Pine palettes and its 24 additional light/dark derivatives
  retain the layered surfaces. Compared with the archived source mapping,
  canonical Main/Moon intentionally change only `border`, `borderMuted`, `dim`,
  `thinkingText`, `selectedBg`, `userMessageText`, `syntaxComment`,
  `syntaxFunction`, `syntaxVariable`, and `syntaxNumber`; canonical Dawn changes
  only the faded `mdLinkUrl` role from `muted` to `pine`. Tests bind the complete
  reviewed files and those explicit customized roles. The compared
  `pi-themes-rose-pine@0.1.0` pack uses the repo's retired simple main mapping
  (plus matching simple moon/dawn variants), so installing it would not fix the
  disliked appearance and would add package state without value. Setup
  atomically selects `theme: rose-pine` on first setup or when the current
  value is outside the managed set. A later setup/update preserves any of the
  three managed variants in
  `~/.pi/agent/settings.json` under Pi's `settings.json.lock` convention.
  Updates from the six-theme trial remove retired `*-fable` aliases only when
  they are recognized repo symlinks or reviewed copies with Git-normalized LF
  or CRLF framing; substantively modified files are preserved.
...
- Native Windows accepts an existing Tree-sitter CLI only when `tree-sitter
  --version` is exactly `0.26.11`. Missing, stale, partial, or incompatible
  commands are repaired from the architecture-specific `v0.26.11` GitHub
...
├── pi/                    # Pi canonical Fable-tuned Rose Pine themes and newline keys
...
| v0.2.0 Linux Nix bootstrap ends with `cat: /etc/bashrc: Permission denied` and upstream's `Oh no` failure | the pinned upstream daemon installer prepared `/etc/bashrc` through its privileged path, then tried to read it as the invoking user; its multi-user path does not honor the public `--no-modify-profile` option | if `/etc/bashrc.backup-before-nix` exists after the failure, restore it with `sudo mv /etc/bashrc.backup-before-nix /etc/bashrc`. For immutable v0.2.0 without that backup, the bounded workaround is `sudo chmod a+r /etc/bashrc` before rerunning. Move to the exact v0.4.2 release checkout and rerun `./setup.sh --all`; its hash-verified local patch skips the daemon profile step and leaves shell activation to setup/Home Manager |
| Linux Nix bootstrap reports `getting status of '/nix/store/...-busybox...': Permission denied` while installing Nix | a restrictive invoking umask—common on managed corporate hosts—combined with Nix 2.34.0's Linux daemon copy and write-bit removal left store directories as root-only `0500`; a previous interrupted attempt can retain those modes | move to the exact v0.4.2 release checkout and rerun `./setup.sh --all` as the normal target user. Its checksum-bound daemon installer normalizes the copied and pre-existing store paths to read-only/traversable modes before Nix creates the default profile; do not run all of setup with `sudo` |
| Corporate Linux bootstrap warns that `https://channels.nixos.org/nixpkgs-unstable` failed TLS verification | upstream's legacy default-channel step uses its bundled CA file, which does not contain the corporate TLS-inspection root; the dotfiles package layer uses locked flakes and does not need this mutable channel | move to the exact v0.4.2 release checkout and rerun `./setup.sh --all`. The wrapper disables channel creation with upstream's public `--no-channel-add`; subsequent Nix activation selects the host system CA bundle, preserving corporate trust without weakening TLS verification |
| Neovim stops before loading Lazy with a lockfile/cache identity or checkout-lock error | `lazy-lock.json` is invalid; the cached `lazy.nvim` checkout fails its origin/branch/commit/cleanliness/entrypoint proof; or a legacy/malformed ownerless lock cannot be proved abandoned | restore the tracked `nvim/lazy-lock.json` and restart Neovim. Startup repairs invalid caches transactionally and automatically reclaims a lock only when its recorded owner PID is provably gone. For an unprovable lock, first confirm no Neovim process owns it, then remove the exact lock path printed by the timeout and restart; never delete a lock held by a live process |
