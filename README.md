...
| tmux/psmux status bar looks washed out or transparent over the desktop | a bar background equal to the terminal's default background renders transparent (the window `opacity` passes through); it was never a color or psmux problem | the status-bar bgs are Rose Pine `surface #1f1d2e` (one shade off `base`), so terminals paint the bar **opaque** while the body keeps its transparency — same on macOS/Linux Ghostty and Windows WT (both ~95% opacity). Update this repo and reopen the terminal |
