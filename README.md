...
| nvim syntax looks weak or CMake looks plain text | the local `nvim-treesitter` checkout may still be on the frozen `master` API while this config expects the `main` rewrite, or parsers were not rebuilt after toolchain setup | run `:Lazy! sync`, then `:TSUpdate`; on Windows prefer re-running `.\setup.ps1 -SkipDeps -SkipConfig` so setup imports the VS DevShell before parser builds |
