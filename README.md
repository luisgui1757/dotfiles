...
| tmux/psmux status bar looks transparent / washed out over the desktop | Windows Terminal applies `opacity` window-wide to every cell, so a transparent WT has a transparent bar regardless of the bar's bg color (a distinct bg does NOT make it opaque in WT) | the fragment ships WT `opacity: 100` (fully opaque) so the dark `base` bar is solid; macOS/Linux Ghostty get an opaque-looking bar from `background-opacity 0.95` + blur. Update this repo and reopen the terminal |
| PowerShell Tab completion selected item is hard to spot | PSReadLine `Selection` color did not stand out | update this repo and open a new `pwsh`; `Selection` is now Rose Pine **gold text** on the subtle overlay highlight so the selected completion pops without a full gold background |
