...
| PowerShell Tab completion colors the inserted suffix (e.g. `.exe`) differently from what you typed | PSReadLine uses one `Selection` color for the menu item AND the completion suffix it inserts; a foreground there splits the line | `Selection` is a **background-only** highlight (`overlay`, no foreground), so the inserted suffix keeps its own color and the menu item is marked by a subtle highlight |
