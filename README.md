...
| nvim syntax looks weak or CMake looks plain text | Tree-sitter is inactive, or C/C++/CMake lost the built-in regex syntax fallback after Tree-sitter starts | update this repo, re-run setup, then check `:Inspect` on a token; C/C++/CMake should show both `treesitter` captures and `syntax` groups |
