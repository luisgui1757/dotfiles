...
| PowerShell Tab completion selected item | PSReadLine `Selection` controls the highlighted row | it is Rose Pine **gold foreground only** — the background blends with the terminal so the gold text marks the selection without a highlight bar. To add a highlight bar, give `Selection` a background too |
