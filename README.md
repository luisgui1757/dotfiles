...
- **tmux Rose Pine differs by platform.** macOS/Linux/WSL `install-deps`
  ...
  current directory basename; Starship shows username, full path, git,
  language/runtime, and time context; host stays out of the daily surface by
  default.
...
- **tmux Rose Pine per runtime.** POSIX tmux loads the upstream `rose-pine/tmux`
  ...
  path, username, git, and runtime state stay in Starship rather than being
  duplicated in tmux; host stays out of the daily surface by default. Windows psmux
