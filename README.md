...
| psmux status bar / inactive titles look washed out or transparent | Windows Terminal transparency (`opacity` < 100 with `useAcrylic` off = sharp, un-blurred see-through) bled a bright/clear desktop wallpaper through the bar — it was never a color or psmux problem | the fragment now ships `opacity: 100` (fully opaque, reliable even in a VM where WT acrylic blur is not); rerun setup, or set `opacity` to `100` in `settings.json`. macOS gets the same readable bar from Ghostty `background-opacity = 0.95` + blur |
| PowerShell Tab completion selected item is invisible or blends in | PSReadLine `Selection` was a dark Rose Pine color, so the highlighted row did not stand out | update this repo and open a new `pwsh`; `Selection` is now a high-contrast gold bar (Rose Pine `base` text on `gold`) so the selected completion pops |
