...
|---|---|---|
| v0.2.0 Nix prerequisite | Upstream Nix `2.34.0`; aarch64-darwin `47cb78c9fdc7b630dbbb9a89869c8e8bcd8c9eb17be036fba18585120693a4c1`; x86_64-linux `5676b0887f1274e62edd175b6611af49aa8170c69c16877aa9bc6cebceb19855`; aarch64-linux `cfddd4008b57a71464a16d5232cba79b1c76ae9dc81bbf71b4972b0118bc29c5` | One isolated official-remote advertisement selects the trust state: before v0.2.0 exists, only a clean exact current official branch head is accepted; once the unique annotated tag appears, branch authority closes and the local tag object, peeled commit, and HEAD must match it. The installer then downloads one versioned `releases.nixos.org` tarball with HTTPS/TLS constraints, compares the review-pinned SHA-256, rejects unsafe archive paths, and only then extracts and executes the fixed installer path with the platform mode, upstream `--yes`, and a local reviewed `nix-command flakes` config. Daemon installs persist it through the upstream config flag; single-user Linux merges the same additive user setting. Other platforms are rejected before download. |
