...
|---|---|---|
| v0.2.0 Nix prerequisite | Upstream Nix `2.34.0`; aarch64-darwin `47cb78c9fdc7b630dbbb9a89869c8e8bcd8c9eb17be036fba18585120693a4c1`; x86_64-linux `5676b0887f1274e62edd175b6611af49aa8170c69c16877aa9bc6cebceb19855`; aarch64-linux `cfddd4008b57a71464a16d5232cba79b1c76ae9dc81bbf71b4972b0118bc29c5` | One isolated official-remote advertisement selects the source identity. The unique annotated release is the default and the local tag object, peeled commit, and HEAD must match it. The explicit `--allow-unreleased` field-test lane may instead accept only a clean checkout whose HEAD equals a current branch head in that same official advertisement; forks, dirty trees, and stale/local-only commits remain rejected. The installer downloads one versioned `releases.nixos.org` tarball with HTTPS/TLS constraints, compares the review-pinned SHA-256, rejects unsafe archive paths, and only then executes the fixed installer path with the platform mode, upstream `--yes`, `--no-modify-profile`, exported `NIX_INSTALLER_NO_MODIFY_PROFILE=1`, and a local reviewed `nix-command flakes` config. The environment seed is required because Nix 2.34.0 does not export the public flag's backing variable before the daemon-installer exec. Setup/Home Manager own shell activation. Daemon installs persist the Nix features through the upstream config flag; single-user Linux merges the same additive user setting. Other platforms are rejected before download. |
