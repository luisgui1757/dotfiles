...
| Pi theme and keys | `pi/rose-pine.json`; `home/dot_pi/agent/themes/rose-pine.json`; `pi/keybindings.json`; `home/dot_pi/agent/keybindings.json`; `scripts/configure-pi-theme.mjs` | all OSes: `~/.pi/agent/themes/rose-pine.json`, `~/.pi/agent/keybindings.json`, plus the `theme` key in `~/.pi/agent/settings.json` | Config files are POSIX symlinks / Windows copies. Keybindings own exactly the upstream `Shift+Enter` / `Ctrl+J` newline pair. Setup merges only `theme: rose-pine` under Pi's compatible lock and preserves all other settings; uninstall removes that key only while it remains the managed value. |
