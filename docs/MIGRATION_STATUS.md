...
| Pi theme | `pi/rose-pine.json`; `home/dot_pi/agent/themes/rose-pine.json`; `scripts/configure-pi-theme.mjs` | all OSes: `~/.pi/agent/themes/rose-pine.json` plus the `theme` key in `~/.pi/agent/settings.json` | Theme file is a POSIX symlink / Windows copy. Setup merges only `theme: rose-pine` under Pi's compatible lock and preserves all other settings; uninstall removes that key only while it remains the managed value. |
