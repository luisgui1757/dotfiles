...
#   git clone --branch v0.4.1 --single-branch https://github.com/luisgui1757/dotfiles.git "${DOTFILES_DEST:-$HOME/dotfiles}"
    ...
    while IFS= read -r -d '' directory; do
        ...
        case "$stage" in
            ...
            prepared|applying|applied|rolling-back|recovery-required)
                echo "  FAIL: unfinished v0.2.0 migration must be resolved before v0.4.1 setup." >&2
    ...
    while IFS= read -r -d '' directory; do
        ...
        case "$stage" in
            accepted|rolled-back)
                ;;
            prepared|applying|applied|rolling-back|recovery-required)
                echo "  FAIL: unfinished v0.3.0 migration must be resolved before v0.4.1 setup." >&2
                echo "        recovery=$directory" >&2
                echo "        new-checkout=$new_checkout" >&2
                echo "        old-checkout=$old_checkout" >&2
                return 1
    ...
    while IFS= read -r -d '' directory; do
        ...
        case "$stage" in
            accepted|rolled-back)
                ;;
            prepared|applying|applied|rolling-back|recovery-required)
                echo "  FAIL: unfinished v0.4.0 migration must be resolved before v0.4.1 setup." >&2
                echo "        recovery=$directory" >&2
                echo "        new-checkout=$new_checkout" >&2
                echo "        old-checkout=$old_checkout" >&2
                return 1
